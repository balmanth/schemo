...
namespace Schemo::Patterns::Tests::Scopes {
  ...
  private:
    ...
    // Flag pattern.
    template<typename T>
    using FlagPattern = FlagScope<T, 0x01, SymbolPattern>;
  public:
    /// <summary>
    ...
    TEST_METHOD(MethodConsumeBefore) {
      ...
      StateScope<char, FlagPattern> rule;
      auto& symbols = context.getSymbols();
      // Check the environment flag.
      Assert::AreEqual<Id>(0x00, source.getOutput().getFlags());
      // Consume 'a'.
      ...
      Assert::AreEqual<Id>(0x01, symbols.get("a")[0].getFlags());
      // Check the environment flag.
      Assert::AreEqual<Id>(0x00, source.getOutput().getFlags());
      // Check failure.
    ...
    TEST_METHOD(MethodConsumeAfter) {
      ...
      Assert::AreEqual(1U, symbols.getSize());
      Assert::AreEqual<Id>(0x01, symbols.get("b")[0].getFlags());
      // Check the environment flag.
      Assert::AreEqual<Id>(0x00, source.getOutput().getFlags());
      // Check failure.
      Assert::IsFalse(rule.consume(source));
